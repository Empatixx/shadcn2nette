  ...
  test('emits defaults, variant maps, and a class lookup', () => {
    ...
    expect(out).toBe(
      [
        ...
        "\t'sm' => 'h-8',",
        ']}',
        `<button class="{$base} {$variantClasses[$variant] ?? ''} {$sizeClasses[$size] ?? ''} {$class}" data-shadcn="button" n:attr="$attrs">`,
        '\t{block content}{/block}',
        '</button>',
        '',
      ].join('\n'),
  ...
  test('emits a static class with passthrough and a content slot', () => {
    ...
    expect(out).toBe(
      [
        ...
        "{default $class = ''}",
        '{default $attrs = []}',
        '<div class="rounded-xl border {$class}" data-shadcn="card" n:attr="$attrs">',
        '\t{block content}{/block}',
        '</div>',
        '',
      ].join('\n'),
